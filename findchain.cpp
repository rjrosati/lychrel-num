...
int main(int argc, char* argv[]) {
    ...
    if (myrank==0) {
        if (argc != 2) {
            std::cout<<"Usage:"<<std::endl<<"\t./findchain [number of digits to search]"<<std::endl;
    ...
    #pragma omp parallel shared(minseed,maxseed) 
    {
        ...
        #pragma omp for schedule(dynamic,(maxseed-minseed)/1000)
        for (ull seed=minseed; seed<maxseed; seed++) {
            ...
            if (steps<STEP_LIMIT+1 && steps > PRINT_LIMIT) std::cout<<std::string(gen_foc(d,seed))<<","<<steps<<","<<std::string(x)<<std::endl;
